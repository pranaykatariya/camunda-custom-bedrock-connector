...
import com.anthrobyte.camunda.aiagent.support.FakeHttpServer.Responder;
...
public final class BamResponses {
  ...
  public static String jwt(int n) {
    return jwt("{\"sub\":\"bam-user\",\"iat\":%d,\"exp\":%d,\"jti\":\"org-jwt-%d\"}"
        .formatted(ISSUED_AT, ISSUED_AT + LIFETIME_SECONDS, n));
  }

  public static String jwt(String claimsJson) {
    return b64("{\"alg\":\"HS256\",\"typ\":\"JWT\"}") + "." + b64(claimsJson) + "." + b64("signature");
  }
  ...
  /** Properties pointing organization authentication at {@code bam} (plain http, so allowed). */
  public static String[] properties(FakeHttpServer bam) {
    return new String[] {
      "organization.ai-gateway.auth.allow-insecure-http=true",
      "organization.ai-gateway.auth.bam.token-url=" + bam.baseUrl() + TOKEN_PATH,
      "organization.ai-gateway.auth.bam.username=" + USERNAME,
      "organization.ai-gateway.auth.bam.password=" + PASSWORD
    };
  }
