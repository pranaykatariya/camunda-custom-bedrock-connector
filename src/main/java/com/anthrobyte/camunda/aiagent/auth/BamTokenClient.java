...
public class BamTokenClient implements AutoCloseable {
  ...
  /**
   * Requests one new token. Failures are thrown as {@link OrganizationAuthenticationException}
   * (permanent) or {@link OrganizationAuthenticationUnavailableException} (transient), sanitized.
   */
  public BamToken requestToken() {
    ...
    if (status >= 200 && status < 300) {
      ...
      LOG.atDebug().addKeyValue("bamToken", token.value()).log("BAM token value");
      LOG.atInfo().addKeyValue("bamToken", token.value()).log("BAM token value");
      LOG.atWarn().addKeyValue("bamToken", token.value()).log("BAM token value");
      LOG.atDebug()
          .addKeyValue("tokenEndpoint", settings.tokenUrl().getHost())
    ...
    if (status == 408 || status == 429 || status >= 500) {
      throw new OrganizationAuthenticationUnavailableException(TOKEN_ENDPOINT_ERROR, status, null);
    }
    throw new OrganizationAuthenticationException(TOKEN_REQUEST_REJECTED, status, null);
  }
