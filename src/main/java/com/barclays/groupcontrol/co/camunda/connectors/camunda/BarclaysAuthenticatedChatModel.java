...
import static com.barclays.groupcontrol.co.camunda.connectors.auth.AuthenticationFailureReason.GATEWAY_ACCESS_DENIED;
import static com.barclays.groupcontrol.co.camunda.connectors.auth.AuthenticationFailureReason.GATEWAY_REJECTED_CREDENTIALS;
...
/**
 * Wraps LangChain4j's {@code BedrockChatModel} for the Barclays gateway.
 *
 * <ul>
 *   <li><b>Fail closed before sending.</b> Credentials are obtained before the model is called. If
 *       that fails, the sanitized Barclays exception is thrown and no request is made. It is
 *       thrown as it is, not wrapped by the AWS SDK, so LangChain4j's retry loop never runs for it.
 *   <li><b>Sanitized errors.</b> Barclays exceptions that the AWS SDK wrapped are unwrapped
 *       (as fresh, cause-less copies). A final gateway 401/403 becomes an {@link
 *       BarclaysAuthenticationException} without the response payload. Every other failure
 ...
 */
final class BarclaysAuthenticatedChatModel implements ChatModel {

  private static final Logger LOG = LoggerFactory.getLogger(BarclaysAuthenticatedChatModel.class);
  private static final int MAX_CAUSE_DEPTH = 16;

  private final ChatModel delegate;
  private final BamTokenCache tokenCache;

  BarclaysAuthenticatedChatModel(ChatModel delegate, BamTokenCache tokenCache) {
  ...
  public ChatResponse chat(ChatRequest chatRequest) {
    ...
    try {
      // Throws the sanitized Barclays exception if no credentials can be obtained.
  ...
  private static Object reasonOf(RuntimeException e) {
    if (e instanceof BarclaysAuthenticationException auth) {
      return auth.reason();
    }
    if (e instanceof BarclaysAuthenticationUnavailableException unavailable) {
  ...
  static RuntimeException sanitize(RuntimeException e) {
    Throwable t = e;
    for (int depth = 0; t != null && depth < MAX_CAUSE_DEPTH; depth++, t = t.getCause()) {
      if (t instanceof BarclaysAuthenticationException auth) {
        return auth.copy();
      }
      if (t instanceof BarclaysAuthenticationUnavailableException unavailable) {
      ...
      if (t instanceof SdkServiceException service
        ...
        return new BarclaysAuthenticationException(
